...
// Test-only route — triggers biweekly summary for all active restaurants immediately
export async function POST() {
  ...
  if (!restaurants?.length) {
    return NextResponse.json({ error: "No active restaurants found" }, { status: 404 });
  }
  ...
  for (const restaurant of restaurants) {
    ...
    const [revenueNow, revenuePrev, topItems, atRisk] = await Promise.all([
      ...
      supabaseAdmin
        .from("orders").select("total_amount")
        .eq("restaurant_id", restaurant.id).eq("status", "completed")
        .gte("created_at", prevStart).lt("created_at", periodStart),
      supabaseAdmin.rpc("get_item_sales_stats", { p_restaurant_id: restaurant.id, p_days: 15 }),
      supabaseAdmin
        .from("customers").select("id, name")
        .eq("restaurant_id", restaurant.id)
        .gte("total_visits", 2).lt("last_visit_date", periodStart).limit(20),
    ]);
...
Keep it direct, positive, and actionable. Address the owner as "you".`;
    ...
    await sendEmail(
      restaurant.email,
      `[TEST] Bi-Weekly Business Summary — ${restaurant.name}`,
      `<div style="font-family:sans-serif;max-width:560px;margin:auto;padding:32px 16px;">
        <div style="background:#fef3c7;border:1px solid #fbbf24;border-radius:6px;padding:8px 12px;margin-bottom:20px;font-size:12px;color:#92400e;">
          ⚠️ This is a test email — the real one sends on the 1st and 15th of each month.
        </div>
    ...
    results.push({ restaurant: restaurant.name, email: restaurant.email, sent: true });
  }
