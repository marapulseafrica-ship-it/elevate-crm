...
async function processSingleRestaurant(restaurant: any) {
...
export const biweeklySummary = inngest.createFunction(
  ...
  async ({ step }: { step: any }) => {
    ...
    // Process in sequential batches of 10 — avoids hammering DB and Claude API
    // with 100 simultaneous requests at midnight
    const BATCH_SIZE = 10;
    for (let i = 0; i < restaurants.length; i += BATCH_SIZE) {
      ...
      await step.run(`process-batch-${batchIndex}`, async () => {
        await Promise.all(batch.map((r: any) => processSingleRestaurant(r)));
      });
    }
  }
);
